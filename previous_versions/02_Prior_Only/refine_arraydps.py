...
def run_storm_refinement(input_path, output_path, config_path, checkpoint_path, sigma_start=0.5, num_steps=100):
    ...
    # Initialize SDE (handles the EDM scaling and denoiser wrapper)
    ...
    wav_padded = torch.nn.functional.pad(wav_tensor, (0, target_len - sig_len))
    ...
    # Truncate schedule to start from sigma_start
    start_idx = (torch.abs(t_steps - sigma_start)).argmin()
    t_steps = t_steps[start_idx:]

    # Inject initial noise (StoRM: Noisy observation)   
    x = wav_padded + t_steps[0] * torch.randn_like(wav_padded)

    # Reverse SDE / EDM Sampler loop (Prior Only, no likelihood constraint)
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    run_storm_refinement(
        ...
        output_path=os.path.join(script_dir, '../../test_samples/refined_real_speech_prior_only.wav'),
