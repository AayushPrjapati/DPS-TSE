...
def run_storm_refinement(input_path, mixture_path, output_path, config_path, checkpoint_path, sigma_start=0.5, num_steps=100, xi=1.0):
    ...
    # --- REVERSE SDE LOOP (WITH UNMASKED MIXTURE GUIDANCE) ---
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        # Unmasked Mixture Guidance (Loss directly on mixture without mask)
        # Pulls denoised output to match raw mixture envelope everywhere
        loss = torch.linalg.norm(torch.relu(denoised.abs() - y_mix.abs()))
        ...
        if i % 10 == 0:
            print(f"Step {i}/{len(t_steps)-1}, sigma={t_cur.item():.4f}, Loss={loss.item():.4f}")
...
if __name__ == '__main__':
    ...
    run_storm_refinement(
        ...
        output_path=os.path.join(script_dir, '../../test_samples/refined_real_speech_mixture_unmasked.wav'),
